...
export default function ProjectContainer({ project }) {
  ...
  return (
    <motion.div
      ref={ref}
      className={`bg-white w-[280px] max-h-[300px] p-5 shadow-lg mt-10 hover:-translate-y-1 transition duration-300 dark:bg-darkBg ${poppins.className}`}
      initial={{ opacity: 0, y: 30 }}
      animate={isInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 30 }}
      transition={{ duration: 0.5, ease: "easeOut" }}
      whileHover={{ scale: 1.05, boxShadow: "0px 5px 15px rgba(0, 0, 0, 0.1)" }}
    >
      <h3 className="text-center font-extrabold text-gray-600 dark:text-darkText text-xl">
        {project.title}
      </h3>
      <div className="flex flex-col gap-6 mt-4">
        <p
          className={`${hankenGrotesk.className} text-center text-sm text-gray-600 dark:text-darkTextLight`}
        >
          {project.description}
        </p>
